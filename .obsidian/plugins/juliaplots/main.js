...
var JuliaPlotsSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("\u2699\uFE0F Plugin settings").setHeading();
    new import_obsidian.Setting(containerEl).setName("Julia executable path").setDesc("Path to the Julia executable. If not set, the plugin will try to use the system default Julia installation.").addText((text) => {
      var _a;
      return text.setPlaceholder("Example: /usr/local/bin/julia").setValue((_a = this.plugin.settings.julia_path) != null ? _a : "").onChange(async (value) => {
        this.plugin.settings.julia_path = value || void 0;
...
async function generateJuliaPlot(params, outputPath, settings) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const julia = (0, import_child_process.spawn)(juliaExecutable, args);
    let stderr = "";
    let stdout = "";
